...
A **hidden Markov model** is a tuple lambda = (A, B, pi):

* **N** hidden states S?,...,S_N; **M** observation symbols v?,...,v_M.
* **A** = (a??), a?? = P(q_{t+1}=S? | q_t=S?).  Rows sum to 1.
* **B** = (b?(k)), b?(k) = P(v_k | q_t=S?).  Rows sum to 1.
* **pi** = (pi?), pi? = P(q?=S?).  Sums to 1.

Durbin's notation unifies this by adding a silent **Begin** state 0 so
that pi? = a_{0,i} and likewise End = 0 for termination.

## 2. The three fundamental problems

### 2.1 Problem 1 -- Evaluation: P(O|lambda)
**Forward algorithm** (Rabiner ?III-A, Durbin ?3.2):

- alpha?(i) = pi? b?(O?)
- alpha_{t+1}(j) = [ Sigma? alpha_t(i) a?? ] * b?(O_{t+1})
- P(O|lambda) = Sigma? alpha_T(i)

Runs in O(N^2T).

### 2.2 Problem 2 -- Decoding: argmax_Q P(Q,O|lambda)
**Viterbi algorithm** (Rabiner eq 30-35; Durbin ?3.2):

- delta?(i) = pi? b?(O?),   psi?(i) = 0
- delta_t(j) = max_i [delta_{t-1}(i) a??] * b?(O_t),   psi_t(j) = argmax_i [delta_{t-1}(i) a??]
- q*_T = argmax_i delta_T(i);  q*_{t-1} = psi_t(q*_t)

Runs in O(N^2T), must be done in **log-space** to avoid underflow.

### 2.3 Problem 3 -- Learning: adjust lambda to maximize P(O|lambda)
**Baum-Welch (EM)** (Rabiner ?III-C; derivation in Stephen Tu's writeup):

- **Backward algorithm**: beta_T(i)=1; beta_t(i) = Sigma? a?? b?(O_{t+1}) beta_{t+1}(j).
- **Posteriors**:
  - gamma_t(i) = alpha_t(i) beta_t(i) / P(O|lambda)       (state occupancy)
  - xi_t(i,j) = alpha_t(i) a?? b?(O_{t+1}) beta_{t+1}(j) / P(O|lambda)   (pair occupancy)
- **Re-estimation**:
  - pi?? = gamma?(i)
  - ??? = Sigma_t xi_t(i,j) / Sigma_t gamma_t(i)
  - b??(k) = Sigma_{t : O_t=v_k} gamma_t(j) / Sigma_t gamma_t(j)

Converges to a local maximum of the likelihood; initialization matters.

### 2.4 Numerical stability
- **Log-space Viterbi**: replace `max_i delta a` with `max_i (log delta + log a)`.
- **Scaled forward-backward**: at each t, rescale alpha_t(i) by c_t = 1/Sigma? alpha_t(i);
  bookkeeping gives log P(O|lambda) = ?Sigma_t log c_t (Rabiner ?V-A).
- **Log-sum-exp trick** for forward in log space:
  log(Sigma exp(x_i)) = x_max + log Sigma exp(x_i ? x_max).
...
- Posterior gamma_t(?) -> soft probability of being inside an island at each
...
supported" in training data; blend lower orders with weights chi_k to
